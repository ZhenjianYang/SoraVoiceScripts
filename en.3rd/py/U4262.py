...
def main():
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x4F4, 5)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_26D")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x21, 0x3C)
    Sleep(500)
    Jc((scpexpr(EXPR_EXEC_OP, "OP_3E(0x16A, 1)"), scpexpr(EXPR_END)), "loc_202")
    ...
    AnonymousTalk(    #0
        "\x07\x05Found \x1F\x6A\x01\x07\x05.\x02",
    )
    ...
    AnonymousTalk(    #1
        (
            "\x07\x05Found \x1F\x6A\x01\x07\x05 in chest.\x01",
            "\x07\x05Inventory full so gave up \x1F\x6A\x01\x07\x05.\x02",
        )
    )
    ...
    AnonymousTalk(    #2
        (
            "\x07\x05[3/36] At best, most choose to describe its features in simple terms:\x01",
            "a dark, cherry-red wood framed with a steel that was painstakingly\x01",
            "etched end to end with florae.\x07\x00\x02",
        )
    )
