...
def main():
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x598, 0)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_3AB")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x0, 0x3C)
    Sleep(500)
    Jc((scpexpr(EXPR_EXEC_OP, "OP_3E(0x201, 1)"), scpexpr(EXPR_END)), "loc_340")
    ...
    AnonymousTalk(    #2
        (
            "\x07\x05I feel blessed to have gotten the chance to work on this series. I feel like\x01",
            "it was a lot of luck to get this chance. -- 'S'\x07\x00\x02",
        )
    )
    ...
    SetMapFlags(0x8000000)
    FadeToDark(300, 0, 100)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x598, 1)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_4AA")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x1, 0x1E)
    OP_73(0x1)
    OP_6F(0x1, 30)
    ...
    AnonymousTalk(    #3
        (
            "\x07\x05You took the time to talk to me even when you're this busy? Heehee.\x01",
            "You're so sweet.\x02",
        )
    )
    ...
    QueueWorkItem(0x0, 0, lambda_5DF)

    def lambda_5F7():
        OP_67(0, 5580, -10000, 7500)
        ExitThread()

    QueueWorkItem(0x0, 1, lambda_5F7)

    def lambda_60F():
        OP_6B(4640, 7500)
        ExitThread()

    QueueWorkItem(0x0, 2, lambda_60F)

    def lambda_61F():
        OP_6C(57000, 7500)
        ExitThread()

    QueueWorkItem(0x0, 3, lambda_61F)

    def lambda_62F():
        OP_6E(380, 7500)
        ExitThread()

    QueueWorkItem(0x1, 1, lambda_62F)
    FadeToBright(1000, 0)
    OP_0D()
    ...
    def Function_6_6CF(): pass

    label("Function_6_6CF")
