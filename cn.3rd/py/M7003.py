...
def main():
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x4D3, 0)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_7F5")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x0, 0x3C)
    Sleep(500)
    Jc((scpexpr(EXPR_EXEC_OP, "OP_3E(0x19B, 1)"), scpexpr(EXPR_END)), "loc_784")
    ...
    AnonymousTalk(    #1
        (
            ...
            "不过现有的数量太多，\x1F\x9B\x01\x07\x00不能再拿更多了。\x02",
        )
    )
    ...
    def Function_9_836(): pass

    label("Function_9_836")

    SetMapFlags(0x8000000)
    FadeToDark(300, 0, 100)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x4D3, 1)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_900")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x1, 0x1E)
    OP_73(0x1)
    OP_6F(0x1, 30)
