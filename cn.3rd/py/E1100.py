...
def main():
    ...
    TalkBegin(0xFE)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x0, 0)), scpexpr(EXPR_END)), "loc_1E54")
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x4BC, 5)), scpexpr(EXPR_END)), "loc_1DAE")
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x4BC, 5)), scpexpr(EXPR_END)), "loc_1F31")
    ...
    NpcTalk(    #7
        ...
        "凯文·格拉汉姆",
        (
            "#1079F（啊……是没有逃掉的客人。）\x02\x03",

            "#1075F（这面孔……好像在哪儿见过似的。\x01",
            "  ……算了，先不管了。）\x02",
        )
    )
    ...
    TalkBegin(0xFE)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x0, 1)), scpexpr(EXPR_END)), "loc_2061")
    ...
    def Function_20_2145(): pass

    label("Function_20_2145")

    TalkBegin(0xFE)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x0, 1)), scpexpr(EXPR_END)), "loc_21D9")
