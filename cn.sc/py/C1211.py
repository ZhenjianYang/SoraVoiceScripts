...
def main():
    ...
    OP_70(0x0, 0x3C)
    Sleep(500)
    Jc((scpexpr(EXPR_EXEC_OP, "OP_3E(0x1FE, 1)"), scpexpr(EXPR_END)), "loc_1E5")
    ...
    SetMapFlags(0x8000000)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x36A, 1)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_365")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x1, 0x3C)
    Sleep(500)
    Jc((scpexpr(EXPR_EXEC_OP, "OP_3E(0x27D, 1)"), scpexpr(EXPR_END)), "loc_2FC")
    FadeToDark(300, 0, 100)
    OP_22(0x11, 0x0, 0x64)
    SetMessageWindowPos(-1, -1, -1, -1)
    SetChrName("")
    ...
    AnonymousTalk(    #3
        "\x07\x00得到了\x1F\x7D\x02\x07\x00。\x02",
    )
    ...
    AnonymousTalk(    #4
        (
            "宝箱里装有\x1F\x7D\x02\x07\x00。   \x01",
            "所持物品已经满了，\x1F\x7D\x02\x07\x00只能放弃了。\x02",
        )
    )
