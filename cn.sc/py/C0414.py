...
def main():
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x32A, 6)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_3CA")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x0, 0x3C)
    Sleep(500)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x32A, 7)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_316")
    ...
    QueueWorkItem(0x8, 1, lambda_289)

    def lambda_2A4():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x258)
        ExitThread()

    QueueWorkItem(0x8, 2, lambda_2A4)
    ...
    Jc((scpexpr(EXPR_EXEC_OP, "OP_3E(0x16B, 1)"), scpexpr(EXPR_END)), "loc_365")
    ...
    AnonymousTalk(    #2
        (
            "宝箱里装有\x1F\x6B\x01\x07\x00。 \x01",
            "所持物品已经满了，\x1F\x6B\x01\x07\x00只能放弃了。\x02",
        )
    )
