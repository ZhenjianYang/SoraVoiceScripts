...
def main():
    ...
    OP_10(0x0, 0x0)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x446, 3)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_188")
    OP_72(0x0, 0x20)
    OP_72(0x0, 0x8)
    OP_6F(0x0, 0)
    OP_10(0xF, 0x0)
    ...
    QueueWorkItem(0x101, 1, lambda_278)

    def lambda_290():
        OP_67(0, 9500, -10000, 5000)
        ExitThread()

    QueueWorkItem(0x101, 2, lambda_290)
    OP_6C(315000, 5000)
    OP_6B(3500, 0)
    OP_6E(262, 0)
    Fade(1000)
    ...
    QueueWorkItem(0x101, 1, lambda_3CE)

    def lambda_3E6():
        OP_6B(3700, 2500)
        ExitThread()

    QueueWorkItem(0x101, 2, lambda_3E6)

    def lambda_3F6():
        OP_91(0xFE, 0x0, 0xFFFFD8F0, 0x0, 0xFA0, 0x0)
        ExitThread()

    QueueWorkItem(0x8, 1, lambda_3F6)
    Sleep(2000)
    FadeToDark(1000, 0, -1)
    OP_0D()
    OP_A3(0x228C)
    ...
    def Function_4_446(): pass

    label("Function_4_446")
    ...
    QueueWorkItem(0x101, 1, lambda_4F5)
    FadeToBright(1000, 0)
    SetPlaceName(0x11D)
    ...
    def Function_5_5E2(): pass

    label("Function_5_5E2")
    ...
    QueueWorkItem(0x0, 1, lambda_68D)

    def lambda_69F():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0x0, 0x3E8)
        ExitThread()

    QueueWorkItem(0x1, 1, lambda_69F)

    def lambda_6B1():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0x0, 0x3E8)
        ExitThread()

    QueueWorkItem(0x2, 1, lambda_6B1)

    def lambda_6C3():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0x0, 0x3E8)
        ExitThread()

    QueueWorkItem(0x3, 1, lambda_6C3)
    Sleep(2000)
    ...
    QueueWorkItem(0x0, 1, lambda_7B9)

    def lambda_7CB():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x3E8)
        ExitThread()

    QueueWorkItem(0x1, 1, lambda_7CB)

    def lambda_7DD():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x3E8)
        ExitThread()

    QueueWorkItem(0x2, 1, lambda_7DD)

    def lambda_7EF():
        OP_9F(0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x3E8)
        ExitThread()

    QueueWorkItem(0x3, 1, lambda_7EF)
    Sleep(2500)
    Fade(500)
