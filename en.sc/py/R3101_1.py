...
def main():
    ...
    ScpFunction(
        ...
        "Function_1_C0",           # 01, 1
        "Function_2_8C4",          # 02, 2
        "Function_3_11B7",         # 03, 3
        "Function_4_120D",         # 04, 4
        "Function_5_1263",         # 05, 5
    )
    ...
    SetChrPos(0xF6, 35300, 0, -32150, 270)
    ...
    QueueWorkItem(0xF6, 1, lambda_3BE)
    ...
    ChrTalk(    #1
        0x101,
        (
            "#1002FFinally shown yourselves,\x01",
            "huh?\x02",
        )
    )
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x240, 1)), scpexpr(EXPR_END)), "loc_7D8")

    ChrTalk(    #2
        0x106,
        "#057FLet's teach 'em a lesson.\x02",
    )
    ...
    ChrTalk(    #3
        0x103,
        (
            "#525FLet's get to the punishing,\x01",
            "shall we?\x02",
        )
    )
    ...
    QueueWorkItem(0x9, 1, lambda_80E)

    def lambda_823():
        OP_92(0xA, 0x101, 0x3E8, 0x1770, 0x0)
        ExitThread()

    QueueWorkItem(0xA, 1, lambda_823)

    def lambda_838():
        OP_92(0xB, 0x101, 0x3E8, 0x1770, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_838)

    def lambda_84D():
        OP_92(0xC, 0x101, 0x3E8, 0x1770, 0x0)
        ExitThread()

    QueueWorkItem(0xC, 1, lambda_84D)

    def lambda_862():
        OP_92(0xD, 0x101, 0x3E8, 0x1770, 0x0)
        ExitThread()

    QueueWorkItem(0xD, 1, lambda_862)

    def lambda_877():
        OP_92(0xE, 0x101, 0x3E8, 0x1770, 0x0)
        ExitThread()

    QueueWorkItem(0xE, 1, lambda_877)
    Sleep(300)
    OP_44(0x9, 0xFF)
    OP_44(0xA, 0xFF)
    OP_44(0xB, 0xFF)
    ...
    SetChrChipByIndex(0x101, 18)
    SetChrChipByIndex(0x107, 21)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x240, 1)), scpexpr(EXPR_END)), "loc_A4C")
    OP_D2(0x70218, 0x70224, 0x1E)
    OP_D2(0x70219, 0x70225, 0x1F)
    OP_D2(0x7021A, 0x70226, 0x20)
    Jump("loc_A6A")
    ...
    SetChrChipByIndex(0xF7, 30)
    Jc((scpexpr(EXPR_EXEC_OP, "OP_42(0x3)"), scpexpr(EXPR_PUSH_LONG, 0x1), scpexpr(EXPR_NEG), scpexpr(EXPR_NEQ), scpexpr(EXPR_END)), "loc_A94")
    OP_D2(0x701E8, 0x701F4, 0x21)
    OP_D2(0x701E9, 0x701F5, 0x22)
    Jump("loc_AA8")
    ...
    ChrTalk(    #4
        0x101,
        "#1002FThat's it, then.\x02",
    )

    CloseMessageWindow()
    Jc((scpexpr(EXPR_EXEC_OP, "OP_42(0x4)"), scpexpr(EXPR_PUSH_LONG, 0x1), scpexpr(EXPR_NEG), scpexpr(EXPR_NEQ), scpexpr(EXPR_END)), "loc_B09")

    ChrTalk(    #5
        0x105,
        "#047FYes, I believe this settles things.\x02",
    )
    ...
    Jc((scpexpr(EXPR_EXEC_OP, "OP_42(0x3)"), scpexpr(EXPR_PUSH_LONG, 0x1), scpexpr(EXPR_NEG), scpexpr(EXPR_NEQ), scpexpr(EXPR_END)), "loc_B45")

    ChrTalk(    #6
        0x104,
        "#030FYes, I believe this settles things.\x02",
    )
    ...
    ChrTalk(    #7
        0x107,
        (
            "#065FI never thought they'd combine\x01",
            "into one super creepy sheep...\x02",
        )
    )

    CloseMessageWindow()
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x240, 1)), scpexpr(EXPR_END)), "loc_BF7")

    ChrTalk(    #8
        0x106,
        (
            "#053FI doubt they'll come back now\x01",
            "that we've bloodied their noses,\x01",
            "but just to be sure...\x02",
        )
    )
    ...
    ChrTalk(    #9
        0x103,
        (
            "#026FGiven what we did to them,\x01",
            "I doubt they'll return. But just\x01",
            "to be sure...\x02",
        )
    )
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x240, 1)), scpexpr(EXPR_END)), "loc_C90")
    ...
    QueueWorkItem(0x9, 1, lambda_D96)

    def lambda_DB4():
        OP_95(0xA, 0x0, 0x0, 0x0, 0x3E8, 0x2710)
        ExitThread()

    QueueWorkItem(0xA, 1, lambda_DB4)

    def lambda_DD2():
        OP_95(0xB, 0x0, 0x0, 0x0, 0x3E8, 0x2710)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_DD2)

    def lambda_DF0():
        OP_95(0xC, 0x0, 0x0, 0x0, 0x3E8, 0x2710)
        ExitThread()

    QueueWorkItem(0xC, 1, lambda_DF0)

    def lambda_E0E():
        OP_95(0xE, 0x0, 0x0, 0x0, 0x3E8, 0x2710)
        ExitThread()

    QueueWorkItem(0xE, 1, lambda_E0E)
    ...
    OP_63(0xD)
    OP_63(0xE)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x240, 1)), scpexpr(EXPR_END)), "loc_ECB")

    ChrTalk(    #10
        0x106,
        (
            "#057F#3PNext time you go there, I'll make\x01",
            "sure you ain't breathin' when you\x01",
            "leave.\x02\x03",

            "#054FNow GET OUTTA HERE!\x02",
        )
    )
    ...
    ChrTalk(    #11
        0x103,
        (
            "#027F#3PNext time, my fluffy friends,\x01",
            "I'll play with you even HARDER.\x02\x03",

            "#024FUnderstood? Now, go!\x02",
        )
    )
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x240, 1)), scpexpr(EXPR_END)), "loc_F5F")
    SetChrChipByIndex(0x106, 32)
    SetChrFlags(0x106, 0x800)
    OP_43(0xF7, 0x2, 0x1, 0x4)
    OP_99(0x106, 0x0, 0x7, 0x7D0)
    ...
    QueueWorkItem(0x9, 1, lambda_F8E)

    def lambda_FAC():
        OP_95(0xA, 0x0, 0x0, 0x0, 0x258, 0x2710)
        ExitThread()

    QueueWorkItem(0xA, 1, lambda_FAC)

    def lambda_FCA():
        OP_95(0xB, 0x0, 0x0, 0x0, 0x258, 0x2710)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_FCA)

    def lambda_FE8():
        OP_95(0xC, 0x0, 0x0, 0x0, 0x258, 0x2710)
        ExitThread()

    QueueWorkItem(0xC, 1, lambda_FE8)

    def lambda_1006():
        OP_95(0xE, 0x0, 0x0, 0x0, 0x258, 0x2710)
        ExitThread()

    QueueWorkItem(0xE, 1, lambda_1006)
    OP_95(0xD, 0x0, 0x0, 0x0, 0x3E8, 0x2710)
    ...
    QueueWorkItem(0x9, 1, lambda_10EF)
    Sleep(100)
    ...
    QueueWorkItem(0xB, 1, lambda_112A)
    Sleep(100)
    ...
    QueueWorkItem(0xC, 1, lambda_114A)

    def lambda_1165():
        OP_8E(0xD, 0x20BC, 0x0, 0xFFFFA8C6, 0x1388, 0x0)
        ExitThread()

    QueueWorkItem(0xD, 1, lambda_1165)

    def lambda_1180():
        OP_8E(0xE, 0x1504, 0x0, 0xFFFF8D6E, 0x1388, 0x0)
        ExitThread()

    QueueWorkItem(0xE, 1, lambda_1180)
    Sleep(1000)
    FadeToDark(1000, 0, -1)
    OP_0D()
    ClearMapFlags(0x2000000)
    ...
    def Function_3_11B7(): pass

    label("Function_3_11B7")
    ...
    def Function_4_120D(): pass

    label("Function_4_120D")
    ...
    def Function_5_1263(): pass

    label("Function_5_1263")
