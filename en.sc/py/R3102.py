...
def main():
    ...
    FadeToDark(300, 0, 100)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x2A0, 6)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_7FA")
    OP_22(0x2B, 0x0, 0x64)
    OP_70(0x1, 0x1E)
    OP_73(0x1)
    OP_6F(0x1, 30)
    ...
    AnonymousTalk(    #3
        (
            "\x07\x05Ooooh, you're one of those completionist types,\x01",
            "huh?\x02",
        )
    )
    ...
    def Function_5_848(): pass

    label("Function_5_848")
    ...
    QueueWorkItem(0x0, 1, lambda_8D2)

    def lambda_8EA():
        OP_6B(3250, 1500)
        ExitThread()

    QueueWorkItem(0x1, 1, lambda_8EA)

    def lambda_8FA():
        OP_6C(270000, 1500)
        ExitThread()

    QueueWorkItem(0x1, 2, lambda_8FA)
    Sleep(1000)
    SetChrName("")
    ...
    Jc((scpexpr(EXPR_GET_RESULT, 0x0), scpexpr(EXPR_PUSH_LONG, 0x0), scpexpr(EXPR_EQU), scpexpr(EXPR_END)), "loc_9A0")
    ...
    Jc((scpexpr(EXPR_GET_RESULT, 0x0), scpexpr(EXPR_PUSH_LONG, 0x0), scpexpr(EXPR_NEQ), scpexpr(EXPR_END)), "loc_9AF")
    EventEnd(0x1)
