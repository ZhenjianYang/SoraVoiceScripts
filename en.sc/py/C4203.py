...
def main():
    ...
    AnonymousTalk(    #1
        (
            "Found \x1F\xA8\x01\x07\x00 in chest.\x01",
            "Inventory full so gave up \x1F\xA8\x01\x07\x00.\x02",
        )
    )
    ...
    def Function_4_38A(): pass

    label("Function_4_38A")
    ...
    QueueWorkItem(0x0, 2, lambda_3CE)

    def lambda_3E6():
        OP_6B(2800, 1500)
        ExitThread()

    QueueWorkItem(0x1, 1, lambda_3E6)

    def lambda_3F6():
        OP_6C(45000, 1500)
        ExitThread()

    QueueWorkItem(0x1, 2, lambda_3F6)
    Sleep(1000)
    SetChrName("")
    ...
    Jc((scpexpr(EXPR_GET_RESULT, 0x0), scpexpr(EXPR_PUSH_LONG, 0x0), scpexpr(EXPR_EQU), scpexpr(EXPR_END)), "loc_49C")
    ...
    Jc((scpexpr(EXPR_GET_RESULT, 0x0), scpexpr(EXPR_PUSH_LONG, 0x0), scpexpr(EXPR_NEQ), scpexpr(EXPR_END)), "loc_4AB")
    EventEnd(0x1)
    ...
    AnonymousTalk(    #6
        "\x07\x05The trick door is tightly shut.\x02",
    )
