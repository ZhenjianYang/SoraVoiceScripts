...
def main():
    ...
    TalkBegin(0x9)
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x405, 7)), scpexpr(EXPR_END)), "loc_5DA")
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x1, 1)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_56A")

    ChrTalk(    #0
        0xFE,
        (
            "Looks like the old man's finally\x01",
            "wised up to the situation.\x02",
        )
    )
    ...
    ChrTalk(    #1
        0xFE,
        (
            "To be honest, I wish we could evacuate\x01",
            "the whole village, but I guess that\x01",
            "isn't gonna happen.\x02",
        )
    )
    ...
    ChrTalk(    #2
        0xFE,
        (
            "Nothing for it, really.\x01",
            "I'll stick around here.\x02",
        )
    )
    ...
    ChrTalk(    #3
        0xFE,
        (
            "Can't just leave the old coot hanging,\x01",
            "you know?\x02",
        )
    )
    ...
    ChrTalk(    #5
        0xFE,
        "Just how obstinate can one old guy be?\x02",
    )
    ...
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x400, 0)), scpexpr(EXPR_END)), "loc_72D")
    Jc((scpexpr(EXPR_TEST_SCENA_FLAGS, MakeScenarioFlags(0x1, 1)), scpexpr(EXPR_EQUZ), scpexpr(EXPR_END)), "loc_6A5")

    ChrTalk(    #6
        0xFE,
        "You gotta be kiddin' me...\x02",
    )
    ...
    ChrTalk(    #7
        0xFE,
        (
            "I've told him over and over that\x01",
    ...
    ChrTalk(    #8
        0xFE,
        (
            "But he doesn't want to hear it,\x01",
            "OF COURSE.\x02",
        )
    )
    ...
    ChrTalk(    #9
        0xFE,
        "Just got to leave him be for now.\x02",
    )
    ...
    ChrTalk(    #10
        0xFE,
        (
            "The old man doesn't believe me when\x01",
            "I tell him that orbments aren't working.\x02",
        )
    )
    ...
    ChrTalk(    #11
        0xFE,
        (
            "*sigh* Just gotta let him stew for\x01",
            "now, I guess.\x02",
        )
    )
